...
import { BackButton, SoundCard, SoundDetail } from '../../components';
...
export const SoundsScreen: React.FC<SoundsScreenProps> = ({ navigation }) => {
  ...
  // Play audio announcement when Sounds screen opens and stop audio when leaving screen
  useEffect(() => {
    AudioService.playWord('Sounds');
  ...
  useEffect(() => {
    ...
    if (isPlaying) {
      if (selectedIndex === null) {
        ...
        timer = setTimeout(() => {
          setSelectedIndex((prevIndex) => (prevIndex !== null ? (prevIndex + 1) % SOUNDS.length : 0));
        }, 3500);
      }
    }
    return () => {
  ...
  const handleNext = () => {
    setIsPlaying(false);
    if (selectedIndex !== null && selectedIndex < SOUNDS.length - 1) {
      setSelectedIndex(selectedIndex + 1);
    }
  };

  const currentSound = selectedIndex !== null ? SOUNDS[selectedIndex] : null;
      ...
      {currentSound !== null ? (
        <View style={styles.detailWrapper}>
          <SoundDetail
            ...
            hasNext={selectedIndex !== null && selectedIndex < SOUNDS.length - 1}
          />
        </View>
      ) : (
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <SoundCard sound={item} index={index} onPress={() => handleSelectSound(index)} />
          )}
        />
      )}
    </SafeAreaView>
